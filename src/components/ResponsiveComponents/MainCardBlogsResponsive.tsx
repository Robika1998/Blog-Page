...
export default function MainCardBlogsResponsive({
  ...
  return (
    <div
      className={`${
        bgColor
      } rounded-md w-[688px] h-[280px] flex transition-all duration-500 ease-out transform ${
        animateSlide
    ...
    >
      ...
      <div className="flex flex-col justify-between py-10 w-full">
        <div>
          ...
          <p className="text-xs text-gray-600">{truncateDescription(description)}</p>
        </div>
            ...
            {[...Array(totalBlogs)].map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  currentIndex === index ? "bg-[#6D9696] scale-110" : "bg-gray-400"
                }`}
